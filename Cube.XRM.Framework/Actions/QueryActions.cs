...
namespace Cube.XRM.Framework
{
    ...
    public class QueryActions
    {
        ...
        public Result getItemRetrieve(Guid Id, string EntityName, string[] Columns)
        {
        ...
        /// <summary>
        /// Gets the items from rollup query.
        /// </summary>
        /// <param name="rollupQuery">The rollup query.</param>
        /// <returns>Result.</returns>
        public Result getItemsFromRollupQuery(EntityReference rollupQuery)
        {
            var watch = Stopwatch.StartNew();
            try
            {
                Entity entity = xrmService.Retrieve(rollupQuery.LogicalName, rollupQuery.Id, 
                    new ColumnSet(new string[] { "fetchxml" }));

                if (entity != null && entity.Attributes.Count > 0)
                {
                    if (entity.Contains("fetchxml"))
                    {
                        return getItemsFetch(entity["fetchxml"].ToString());
                    }
                    else
                        return new Result(false, "There is not any result.", null, logSystem);
                }
